...
function updateVersionInfo() {
    ...
    const version = htmlElement.getAttribute('version');
    if (version) {
...
async function loadConfigs() {
    try {
        const fieldMapping = window.__LOAD_CFG__ ? window.__LOAD_CFG__('config/field_mapping.json') : await fetch('config/field_mapping.json').then(r => r.json());
        const defaultConfig = window.__LOAD_CFG__ ? window.__LOAD_CFG__('config/default_config.json') : await fetch('config/default_config.json').then(r => r.json());
        const bomRules = window.__LOAD_CFG__ ? window.__LOAD_CFG__('config/bom_rules.json') : await fetch('config/bom_rules.json').then(r => r.json());
        const materialData = window.__LOAD_CFG__ ? (() => { const b=atob(window.__CFG_DATA__['config/material_data.json']); const u=new Uint8Array(b.length); for(let i=0;i<b.length;i++) u[i]=b.charCodeAt(i); return new TextDecoder().decode(u); })() : await fetch('config/material_data.json').then(r => r.text());
        const productionPlans = window.__LOAD_CFG__ ? window.__LOAD_CFG__('config/production_plans.json') : await fetch('config/production_plans.json').then(r => r.json());
        const processRoutes = window.__LOAD_CFG__ ? window.__LOAD_CFG__('config/process_routes.json') : await fetch('config/process_routes.json').then(r => r.json());
